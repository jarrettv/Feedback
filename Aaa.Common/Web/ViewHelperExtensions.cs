...
namespace Cts.Chronos.Web
{
    ...
    {
        public static string EntryUrl(this UrlHelper url, string user, DateTime day, DateTime? month = null)
        {
            month = month ?? day;
            int? cal = day.ToStartOfMonth().CompareTo(month.Value.ToStartOfMonth());
            if (cal == 0) cal = null;
            return url.RouteUrl("Entry", new { user, day.Year, day.Month, day.Day, cal });
        }
    }
}